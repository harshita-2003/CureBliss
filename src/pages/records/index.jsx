...
import CreateRecordModal from "./components/create-record-model";
...
const Index = () => {
  ...
  // Fetch user data and records when user is available
  useEffect(() => {
    if (user) {
  ...
  // Update local state and persist records in localStorage
  useEffect(() => {
  ...
  const createFolder = async (foldername) => {
    try {
      if (currentUser) {
        ...
        if (newRecord) {
          // Refresh user records after creating a new one
          fetchUserRecords(user.email.address);
          handleCloseModal();
        }
      }
  ...
  // Navigate to a selected record's detail page
